...
static PlatformCapability capabilities[] = {
    ...
    { "isolation", "process kill/crash handling", PLATFORM_PARTIAL, "user exceptions stop the app; kernel exceptions still panic" },
    ...
    { "scheduler", "preemptive multitasking", PLATFORM_PARTIAL, "timer IRQ preempts ring-3 apps back to the loader" },
    ...
    { "app api", "create window", PLATFORM_PARTIAL, "SYS_WINDOW_CREATE allocates kernel-owned handles" },
    { "app api", "draw text/shapes/images", PLATFORM_PARTIAL, "draw syscalls validate handles; rendering is pending" },
    { "app api", "input events", PLATFORM_PARTIAL, "SYS_POLL_EVENT exists; routing is pending" },
